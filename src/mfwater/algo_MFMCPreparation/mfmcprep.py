...
def Estimate_eval(path: str | Path , budget: float) -> int:
    ...
    with h5py.File(path, "r+") as f:
        ...
        differences = np.array([(correlation[q]**2 - correlation[q+1]**2) for q in range(n_models)])
        ratio = differences /weights
        r = np.sqrt(ratio/ratio[0])
        high_fidelity_eval = budget/np.dot(weights,r)
        evaluations = np.floor(np.concatenate(([high_fidelity_eval], r[1:]*high_fidelity_eval))).astype(int)
        if evaluations[0] <1:
            raise ValueError("Increase the budget. The high-fidelity model has to be evaluated at least once.")
...
def MultifidelityMonteCarlo(path: str | Path)-> float:
    ...
    with h5py.File(path, "r") as f:
        ...
        alpha =[models[highfidelity_model].attrs["variance"]*(models[mod].attrs["correlation"]/models[mod].attrs["variance"]) for _,mod in models.items()]
        prev_mod = None
        for k, (name, mod) in enumerate(models.items()):
            ...
            prev_mod = name
        s = sum(alpha[l] * (MC_estim[l] - MC_estim_pre[l]) for l in range(1, models.attrs["n_models"])) + MC_estim[0]
